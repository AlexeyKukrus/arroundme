<script lang="ts">
	...
	// Обработчик изменения isOpen для блокировки прокрутки фона
	$: {
		if (isOpen) {
			document.body.style.overflow = 'hidden';
			// Добавляем обработчик события клавиатуры при открытии
			if (typeof window !== 'undefined') {
				window.addEventListener('keydown', handleKeydown);
			}
		} else {
			document.body.style.overflow = '';
			// Убираем обработчик события клавиатуры при закрытии
			if (typeof window !== 'undefined') {
