...
## MOCK API SERVER - ВРЕМЕННЫЙ

Для локальной разработки можно запустить мок-сервер, не меняя существующие методы в клиенте.

1. Установите зависимости сервера (используется express и cors – уже включены в dev deps vite, если нужно, установите):

```
npm i express cors --save-dev
```
...
3. Укажите базовый URL API через переменную окружения для Vite:
...
Либо задайте переменную окружения при запуске dev-сервера.

Клиентские методы используют `fetchFromClient`, который автоматически подставляет `VITE_API_BASE_URL`.
